...
function mise {
    ...
    if ($arguments.count -eq 0) {
        & C:\Users\Aaron\AppData\Local\Microsoft\WinGet\Links\mise.exe
        _reset_output_encoding
        return
    } elseif ($arguments -contains '-h' -or $arguments -contains '--help') {
        & C:\Users\Aaron\AppData\Local\Microsoft\WinGet\Links\mise.exe @arguments
        _reset_output_encoding
        return
    }
    ...
    switch ($command) {
        { $_ -in 'deactivate', 'shell', 'sh' } {
            & C:\Users\Aaron\AppData\Local\Microsoft\WinGet\Links\mise.exe $command @remainingArgs | Out-String | Invoke-Expression -ErrorAction SilentlyContinue
            _reset_output_encoding
        }
        default {
            & C:\Users\Aaron\AppData\Local\Microsoft\WinGet\Links\mise.exe $command @remainingArgs
            $status = $LASTEXITCODE
...
function Global:_mise_hook {
    if ($env:MISE_SHELL -eq "pwsh"){
        & C:\Users\Aaron\AppData\Local\Microsoft\WinGet\Links\mise.exe hook-env $args -s pwsh | Out-String | Invoke-Expression -ErrorAction SilentlyContinue
    }
}
        ...
        $_mise_pwsh_cmd_not_found_hook = [EventHandler[System.Management.Automation.CommandLookupEventArgs]] {
            param([object] $Name, [System.Management.Automation.CommandLookupEventArgs] $eventArgs)
                ...
                if ([Microsoft.PowerShell.PSConsoleReadLine]::GetHistoryItems()[-1].CommandLine -match ([regex]::Escape($Name))) {
                    if (& C:\Users\Aaron\AppData\Local\Microsoft\WinGet\Links\mise.exe hook-not-found -s pwsh -- $Name){
                        _mise_hook
